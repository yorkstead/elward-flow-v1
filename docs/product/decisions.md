...
## 2026-08-31 — Authentication and operational safety repair

- Removed embedded owner credentials and the default signing secret. Passwords must match an active provisioned account. Existing JWT sessions recheck the account and current roles, so removed, disabled, or fabricated users cannot retain access through stale claims.
- Passkey registration and authentication require user verification. Challenges are bound to the browser, purpose, and registration user, expire after five minutes, and are consumed atomically from PostgreSQL. APP_URL supplies the trusted origin and relying-party ID.
- Scanner execution validates permissions, organization, current approved revision, operation ownership, predecessor completion, action, condition, remaining quantity and current state before recording a mutation. Starting work does not count completed pieces. Ambiguous legacy mark names fail rather than selecting an unrelated job; exact mark UUIDs remain resolvable.
